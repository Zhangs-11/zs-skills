...
name: human-writing
description: |
  通用中文创作与改稿 Skill。用于知乎回答、论坛长帖、公众号文章、博客、评论、人物故事、历史叙事、新闻与行业解读、科普、教程、评测、个人叙事、小说、故事、对白、口播和演讲稿。默认写成一个见过事、查过材料、愿意把来龙去脉讲清楚的人在说话，重点保留中文互联网长回答与长帖的活人感和自然中文韵律，避免空泛的机构腔、喊口号式演说腔、营销腔和模型腔。非虚构长文先检查材料够不够，材料不足时研究、追问或缩短，绝不用重复解释灌字数。现实内容额外核验事实、引语、数据与用户亲历，虚构内容可以创造人物、场景、对白、心理与情节。独立使用时执行本 Skill 的成稿禁令；与更具体的个人、品牌或文体 Skill 同时使用时，由上游 Skill 调度正文任务并覆盖表层文风规则，事实边界始终不可覆盖。本 Skill 不创建作者画像、个人规则库或个人写作 Skill。
---
...
本 Skill 可以独立完成一篇作品，也可以被更具体的个人、品牌或文体 Skill 调度。

被上游 Skill 调度时：

- 接收上游已经确认的作者位置、真实材料、核心判断、读者和交付边界。
- 负责正文的材料密度、事实边界、自然中文和段落推进。
- 不擅自创建个人画像、标题、封面、CTA、发布说明和内部质检报告，除非上游明确要求。
- 上游可以覆盖冒号、破折号、对比句、段落长度等表层风格禁令。
- 用户亲历、事实、来源、现实与虚构边界不能被任何上游文风规则覆盖。
...
3. 同时生效的更具体个人、品牌或文体 Skill 对表层风格和交付形式的规定。
...
下面几项是本 Skill 独立写作时的默认成稿规则。被更具体的个人、品牌或文体 Skill 调度时，上游可以覆盖表层句式和标点禁令；涉及真实与虚构、来源和用户亲历的规则仍然不可覆盖。
...
长稿可以保存为 Markdown 或文本后运行 `scripts/check_prose.py <稿件路径>`。独立使用时，脚本失败就继续修改，直到禁用项清零。被上游 Skill 调度时，脚本结果只作提醒，由上游规则决定哪些表层命中可以保留。
