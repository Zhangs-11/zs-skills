...
description: 在现有仓库实现功能、修复 Bug 或修改行为、配置及接口时使用；仅修正拼写、格式且不影响行为的局部编辑不加载。
---
...
- 仓库若提供专用开发 Skill 或规范工具，按当前任务读取并组合使用，但不得让其自动扩大本次写入范围。
...
每次改完代码，自审都要逐条回答下面四问。每问给出查过的证据，或者写明查了哪些地方、结论是「未发现」；只写一句「已自审」不算完成：

1. **有没有过度设计**：把本次新增的抽象、状态、字段、分支、配置和文案逐项试着删掉或合并，删掉后验收、失败语义和测试保护不变的就删；刻意保留的要写出对应的需求或契约。
2. **有没有逻辑错误**：本次代码响应的每个信号（事件、状态、错误、取消、重试、回调），都要列出它在项目里的全部触发来源和取值，不能只看本次想处理的那一种；逐个来源走一遍分支，误触发和漏触发都算错误。
3. **有没有完整考虑整条链路**：从触发一直追到用户可见结果，确认每个会改变结果的状态变化都能传到本次改动的代码，包括不在实时通道上的对象、刷新或重连后的恢复、分页以及并发到达的数据。
4. **会不会影响已有功能**：本次新增或改变的每个输出（事件、字段、枚举值、状态、接口、文案），都要搜出现有的全部消费方，包括其他仓库的 SDK、模板和契约文档，逐个核对它们按现有语义会怎样理解这个输出；同名取值的含义以消费方契约为准。如果要改的是他人负责的代码，先按仓库规则确认负责人，再决定改不改。
...
- 运行了什么验证，结果如何；
- 自审四问（过度设计、逻辑错误、整条链路、已有功能）逐条的结论与证据，发现并自动修复了什么，最终复审结论是什么；
- 仍有哪些会影响正确性的未验证事实。
