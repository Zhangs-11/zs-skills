---
name: peer-pr-review
description: 审查同事或自己的代码改动，并把陌生需求、关键术语和方法调用整理成新手也能看懂的 Review，再用一个具体 Case 从输入到结果走通改动前后的真实链路。用户要求 review、自审、检查本地 Git 改动、暂存区、未提交文件、当前分支、独立 worktree 或 CodeUp PR 时，应使用本 Skill；先固定基线与真实 diff，再核对上下游、项目规范和可用运行时证据，独立判断问题、根因、改动合理性与遗漏。只读审查，不自动修改、提交、推送、评论、通过或合并。
---
