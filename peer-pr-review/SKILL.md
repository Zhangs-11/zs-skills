---
name: peer-pr-review
description: 用户要求审查 PR、分支、worktree 或本地改动，或实现完成需自审时使用；只读核对完整 diff、需求与证据，并用具体 Case 解释结论。
---
...
如果仓库或所在组织提供了专用开发规范、知识库导航或当前的根因与证据合同，按需读取最新版本；这些资料用于形成和校验假设，不能代替本次审查对象的代码与运行证据。
...
审查材料里“没有可用资产”“环境不支持”“无法复现”“等部署后再验”这类说法，同样是待证伪的主张，先自己核实，再决定是否写进“未验证”。依次确认三件事：改动是否已部署到可访问的环境（镜像 tag 对照 commit）；库里是否已有同类型的现成资产或测试夹具（只读查询）；能否用只读、无副作用的调用走通目标路径。路径可行就当场实测，实测与静态推断冲突时以实测为准；实测需要写数据、新建资产或触及生产时，先说明拟做的操作并取得用户同意。核实后仍不可行，才写进“未验证”，并注明查过哪些途径、卡在哪一步。
...
| 触发条件 | 必读检查 |
|---|---|
| 已产生持久副作用后仍可能返回失败 | [部分成功与同资源恢复](references/partial-success.md) |
| 新增或收紧校验、解析、Schema、序列化、路由或兼容层 | [跨层契约等价性](references/cross-layer-contract.md) |
| 修改 MCP、OpenAPI、RPC 等对外 Schema | [广播必传与运行时校验](references/schema-required.md) |
| 返回或上报 actual/effective/final/runtime 等事实字段 | [实际值来源](references/actual-fields.md) |
| 多入口、角色、协议分支或可共存状态 | [验收覆盖矩阵](references/acceptance-matrix.md) |
| 重试、重连、取消、超时、流式响应或错误正文提取 | [重试与取消](references/retry-cancellation.md) |
...
输出前把每条建议按审查改动的同一标准过一遍：估算触发概率和实际损失，衡量它新增的状态、依赖和维护面，低概率边角隐患按“过度设计门禁”搁置。与本轮已确认的设计口径、需求条目或上一轮自己给出的结论冲突的建议直接撤回，不靠新增一层机制去补上一轮刚删掉的东西。
...
未验证：<只列可能改变结论的缺口，每项注明已排查的验证途径和卡点；没有则写“无关键缺口”>
