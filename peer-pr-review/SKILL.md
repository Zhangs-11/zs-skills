---
name: peer-pr-review
description: 审查同事或自己的代码改动，并把陌生需求、关键术语和方法调用整理成新手也能看懂的 Review，再用一个具体 Case 从输入到结果走通改动前后的真实链路。用户要求 review、自审、检查本地 Git 改动、暂存区、未提交文件、当前分支、独立 worktree 或 CodeUp PR 时，应使用本 Skill；先固定审查范围，在阅读 patch 方案前按需求独立形成开发方案，再对照真实 diff、项目规范和运行证据判断根因、方案合理性与遗漏。只读审查，不自动修改、提交、推送、评论、通过或合并。
---
...
|---|---|---|---|---|---|
| `<新入口/新来源>` | `<已运行的等价入口>` | `<第一个不同的字段/状态/分支>` | `<可转成已有契约的位置>` | `<有证据的最小改动层>` | `<已复用什么>` |
...
完成需求、基线机制、项目参考和独立根因判断后，先假设这个需求现在交给自己开发，形成一份**独立开发方案**。这一阶段不读取 patch hunk，也不采用改动者的根因、文件选择和测试结论；如果 PR 页面把问题与方案写在一起，只抽取可由任务事实核验的问题描述，把方案部分留到下一阶段。若没有独立任务材料，需求只能从 patch 恢复，明确标注“独立方案受 patch 信息影响”，不能把它包装成真正的 diff-blind 方案。

独立方案不是一份脱离项目的架构畅想，而是复用日常开发流程得到的最小交付蓝图：

对于现有代码任务，完整读取并在只读边界内使用 `$project-aware-coding`；按实际触发条件再组合诊断、TDD、安全、协议或领域 Skill。复用这些 Skill 的目标确认、调用链、项目参考、方案设计、验证和自审标准，但不执行其中的代码修改、自动修复或外部写操作。这样比较的是“如果按同一套开发纪律交付，我会选择什么方案”，不是 reviewer 临时凭印象写出的理想答案。

1. **调查与 Skill 路由**：需要哪些诊断、项目开发、协议、安全或领域 Skill；要查哪些代码、配置、日志、数据库、历史和运行证据。只写会改变方案的选择，不罗列形式化工具清单。
2. **机制与改动归属**：首次偏离在哪里，最小应修改哪一层、哪些生产者和消费者，哪些层能复用既有契约所以不改。
3. **实现方案**：需要调整的入口、状态、协议、错误处理和终态；覆盖正常、失败、恢复、重复、并发及权限边界中真实可达的部分。
4. **验证方案**：哪些单元、集成、契约和真实 QA 证据才能证明旧问题被抓住、正常路径未回归、副作用次数正确；配置、工具可见性、部署或环境差异必须指定运行时事实源。
5. **自审闭环**：假设实现完成后，应基于完整 diff 独立自审，修复原需求范围内有证据的必须项，重新验证并全量复审；这里只用它检查改动者方案是否具备同等交付完整性，Review 本身仍不修改代码。
...
|---|---|---|---|---|---|---|
| ① | `<关键字段或状态>` | `<repo/path:line>` · `<method()>` | `<先用业务白话解释职责>` | `<判断、状态变化或副作用>` | `<判断、状态变化或副作用>` | `<下游输入或用户可见结果>` |
...
<先概括如果由 reviewer 开发会选择的最小方案、验证与自审闭环，再说明它与实际方案如何互相补漏；简单一致时压缩成一段。>

| 对照维度 | 独立开发方案 | 改动者方案 | 复核结论与证据 |
|---|---|---|---|
| 改动层与机制 | <应改哪里、复用什么> | <实际怎样改> | <一致/谁更完整/仍缺什么> |
| 失败与恢复 | <应覆盖的关键路径> | <当前覆盖> | <运行测试/静态调用链/未验证> |
| 验证与自审 | <应有的测试、QA、复审> | <作者证据> | <是否足以改变合并决定> |
...
- 背景、问题和根因没有互相重复或偷换；
- 已在阅读 patch 方案前基于任务、基线代码和真实机制冻结独立开发方案，没有从改动位置倒推自己的设计；
- 独立方案已覆盖会影响交付的 Skill/调查路线、改动归属、正常/失败/恢复、验证和自审闭环，但没有把 Review 变成实际修改；
- 已将独立方案与改动者方案逐项对照，既寻找改动者遗漏，也吸收其比独立方案更完整的设计和证据；
- 方案差异本身没有被当作 finding；每个候选问题都经过双方反例与运行证据证伪，静态推断被真实成功调用推翻时已撤回；
- 改动者的判断被当作假设独立核验；
...
- 任一持久副作用发生后仍可能返回失败的路径，已证明安全清理或真实可执行的同资源恢复；同输入重试不会制造重复对象或重复副作用；
- 共享配置被多个构造器配合不同语义参数消费时，已枚举生产调用方，并以默认和非默认消费者验证配置归属没有串到当前资源；
- 方法用途来自真实实现和上下游，不是根据名字或注释猜测；
- 已使用一个具体 Case 和同一份输入走通改动前后链路，说明初始状态、每一步的判断或状态变化、首次分叉和最终结果；Case 来源与运行验证状态没有被包装或省略；
- 具体 Case 中的步骤、方法、文件和版本能与真实调用链及代码证据对应；简单改动也有最小 Case，复杂改动没有为了控制篇幅截断必要因果链；
- 每个正式问题都有位置、可达路径、触发条件和影响；
- 已指出项目内最接近的同类实现、测试或历史修复，写明复用或偏离结论；没有可靠样本时已记录实际搜索范围；
- 新增或收紧校验、解析、Schema 或兼容逻辑时，已写出真实消费者操作，并双向检查“上游放过/下游拒绝”和“下游接受/上游误拒绝”；
- 对外 Schema 的 `required` 广播契约与服务端运行时校验已分层核对；必传字段已通过真实 transport 或 `Tool.run()` 路径验证，未根据测试注释或 `.fn()` 直调推断；
- 跨语言、跨库或跨版本的同一概念已用同一组标准、兼容、非法和边界样例对照，未从同名方法或测试绿灯推断契约一致；
- 返回体、日志和审计中的 `actual` / `effective` / `final` / `runtime` 字段已追到所声明的真实事实源；回读不可得时显式留空并标注来源，未用请求值或配置值静默回退；
...
- 已对新增抽象、状态、helper、后台任务、缓存、兼容层、重试和配置项尝试删除、合并或复用；不存在同样满足验收与失败语义的更小方案；
...
- 输出让零背景读者无需重新考古就能听懂这一条 Case，并据此决定是否通过、退回或补证。
