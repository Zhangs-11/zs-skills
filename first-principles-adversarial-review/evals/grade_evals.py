...
def grade(eval_id: int, text: str, events: str) -> list[dict[str, object]]:
    ...
    rules: dict[int, list[tuple[str, bool, str]]] = {
        1: [
            ...
            ("标注未验证信息", contains_any(text, ["推断，未验证", "未验证项", "尚未验证", "无法核对", "需要确认"]), "回答区分事实与待核实项"),
        ],
        2: [
            ("自动读取 Skill", skill_read, "事件日志中出现 SKILL.md 读取路径"),
            ...
            ("给出证伪检查", contains_any(text, ["证伪", "验证", "核验", "对照", "时间线", "指标"]), "回答包含可验证路径"),
        ],
        3: [
            ("自动读取 Skill", skill_read, "事件日志中出现 SKILL.md 读取路径"),
            ("拒绝因果跳跃", contains_any(text, ["不能说明", "不能证明", "不成立", "无法判断", "无法归因", "无法将", "相关性", "因果"]), "回答质疑注册上涨直接归因"),
        ...
        7: [
            ("自动读取 Skill", skill_read, "事件日志中出现 SKILL.md 读取路径"),
            ("区分主张类型", contains_any(text, ["事实", "推断", "价值判断", "规范性"]), "回答区分事实、推断或价值判断"),
            ("拒绝因果与范围跳跃", contains_any(text, ["不能推出", "不能证明", "相关性", "因果", "所有公司", "适用范围"]), "回答检查因果和全称推广"),
            ("使用证据等级", contains_any(text, ["已证实", "需要收窄", "存在争议", "证据不足", "明显错误"]), "回答使用与证据相称的核查等级"),
        ],
    }
    return [
