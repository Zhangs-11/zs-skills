...
8. 否定替代方案的理由（某个库或接口做不到、要多引依赖、成本更高）是否经过同样核验，而不是凭记忆判断？
9. 把问题定性为设计缺陷之前，是否已核对现有流程、部署规范和团队约定？如果按约定执行本就不会出问题，根因是偏离约定，而不是设计缺陷。
10. 修复是否让原本从不执行的代码开始执行（例如字段终于读到值、开关终于生效、条件终于成立）？是的话，把被激活的代码当作新上线代码审查：列出所有会触发它的真实生产者及其实际数据形状，逐一对照它的判断和失败处理，并至少用一个修复目标以外的真实生产者跑通；只证明修复本身读对、写对，不能推出“对其他路径没有影响”。
11. 读代码下结论前，是否确认了这个服务线上实际从哪个仓库、哪个分支构建？同名或相似的仓库可能已经停更，本地目录名不能当作证据，要按仓库地图、部署记录或镜像来源核对。给线上问题定根因或动手修复前，是否在任务系统里搜过同一问题？已有任务里的根因、设计约定和在途分支都是要先证伪的证据，不能跳过。
