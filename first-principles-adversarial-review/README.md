...
这项 Skill 按机制推导与反证需求触发，不必用于每个简单任务。如需全局路由，可在 `AGENTS.md` 或 `CLAUDE.md` 中加入：

```markdown
需求设计、根因诊断、复杂实现或关键事实争议需要机制推导与反证时，使用 `$first-principles-adversarial-review`；简单查询、忠实摘要和不改变行为的局部修订只核对直接相关材料。
```
...
| 故障诊断、性能和根因分析 | 使用 | 使用 |
| 影响行为的复杂实现、配置或流程变更 | 使用 | 使用 |
| 关键事实争议或有歧义的数据解读 | 按机制需求 | 使用 |
| 简单查询、忠实摘要、不改变行为的局部修订 | 不加载 | 只核对直接事实 |
...
- [ ] 如需全局路由，在 `AGENTS.md` 或 `CLAUDE.md` 中加入上面的条件入口。
...
测试覆盖技术方案、故障归因、运营因果判断、删除需求、已确定技术约束、观点主张分类和简单翻译负例。此前版本的七类用例在隔离的 Codex 新会话中完成 27 条行为断言，全部通过（不代表本次条件路由已通过行为回归）；最易欠触发的正例重复 3 次全部加载，翻译负例重复 3 次全部跳过。
...
| 问题 | 原因 | 解决方法 |
|---|---|---|
| 复杂任务未触发 | 当前会话未刷新，或触发条件不清 | 新开会话，核对上面的条件路由；简单任务跳过是预期行为 |
