...
```text
用户：请展开双向钢人，分析是否应该把所有内部服务都改成事件驱动。
```
...
```markdown
每个新任务先使用 `$steelman-before-answer` 在后台完成正反钢人和关键变量审查，不默认展示过程。无需用户选择时直接回答或执行；只有会实质改变结果的用户选择才说明影响并问一个原子问题。用户明确要求展开时才展示完整论证。
```
...
| 问题 | 原因 | 解决方法 |
|---|---|---|
| 每次仍展示完整正反论证 | 全局规则仍要求可见五段模板 | 改为“后台完成，明确要求时才展开” |
