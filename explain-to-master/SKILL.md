...
description: 运用费曼学习法（teach-back）、苏格拉底式单问题追问、具体 Case 走读、证据核验、渐进提示、反例和迁移测试，帮助用户真正掌握任意主题。用于用户表示“我好像懂了但讲不清”“看完分析还是似懂非懂”“用一个具体 Case 带我走一遍”“考考我或检查理解”“帮我快速弄懂这段代码、文档、业务、流程或概念”“让我能向别人讲清楚”，以及刚完成或接手陌生工作却尚未形成可靠心智模型时。适用于代码、技术系统、论文、课程、产品与业务机制、操作流程和决策方案；不用于用户只要直接答案、普通摘要或代写且没有学习意图的请求。
---
...
不要机械询问用户想选哪种模式；能从对话判断时直接开始。只有存在多个差异很大的 Case，选择会改变要学习的机制，而且材料无法确定用户卡点时，才一次追问一个最关键问题。
...
默认采用标准模式：事实核验、数轮追问、一次迁移验证。用户要求“用 Case 带我走一遍”或看完分析仍似懂非懂时采用具体 Case 走读模式：先直接走通一个最小完整链路，再进行一个单问题检验，根据回答决定是否继续。用户要求“快速”时采用快速模式，只选一个关键机制，完成一次具体走读或复述、一个漏洞修复和一个新情境验证。用户准备承担重要决策、教学、评审或高风险操作时采用深入模式，增加来源核验、替代方案、边界和故障推演。
