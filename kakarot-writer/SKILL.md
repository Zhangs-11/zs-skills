...
2. **过度结构化**：不用bullet point罗列观点，不大量加粗。小标题改为「可选用」——短文、强叙事（调查实验型/产品体验型那种一镜到底的）仍然不用小标题，从头到尾一口气顺下来，靠节奏和转场自然推进；只有信息量大、需要分区的长文（现象解读/方法论那种）才用小标题（用 `## 二级标题`，发布时会渲染成醒目的图标+蓝色色块样式）。用不用由作者按这篇的结构定，默认倾向不用。"N条心得/方法"这类独立条目仍可用数字编号（1、2、3）。无论用不用小标题，板块之间都要有口语化转场句（"说到这个""回到xxx这块"）兜着，不能光靠标题硬切
3. **标点禁令**：
   - 不使用冒号"："，用逗号代替
   - 不使用破折号"——"
...
- **小标题可选用**。短文/强叙事一口气顺到底，不用小标题；信息量大的长文可以用 `## 二级标题` 分区（发布会渲染成醒目的图标+蓝色色块样式，标题里可自带一个语义 emoji，没带会自动补 🔹）。无论用不用，板块之间都要有口语化转场句（"说到这个""回到xxx这块""顺着上面的再聊聊"）兜着。"N条心得"这种分条目结构用数字编号
- **关键小句可轻强调**。代替加粗——把一篇里最值得停一下的那几句关键判断，用 `==小句==`（双等号）包起来，发布时会渲染成克制的浅蓝荧光笔下划线，不抢戏。**克制用**，一篇顶多三五处，多了就跟没标一样。普通加粗仍然少用
