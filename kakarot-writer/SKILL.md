...
- 需要插图的位置，用 `[插图：中文描述]` + `[绘图提示：英文prompt]` 两行占位（配合 wechat-publisher 发布）。**绘图提示要自己手写、有创意**：用一个具体的视觉比喻去概括这一段的意思，而不是把正文照搬进去。纯英文，画面里不要出现任何文字（中文一旦进 prompt 会被画成错别字）。例：解释「海量上下文被收束成清晰推理」→ `A vast turbulent cloud of glowing particles funneling through a sleek device into one calm focused beam of blue light, flat editorial illustration, no text`
