...
- **配图优先用真实截图**。凡是引用了推文、GitHub issue、产品页面、研究报告的段落，用 `[插图：用户截图 - XXX]` 占位并提供原始链接，让用户自己截图。截图下方紧跟一行引用格式的来源地址，地址前必须加带平台名的类型说明前缀，如 `> GitHub地址：github.com/obra/superpowers/issues/190`、`> X推文地址：x.com/xxx/status/xxx`、`> xAI官网公告地址：x.ai/news/xxx`、`> Reddit帖子地址：reddit.com/xxx`，让读者一眼看出来源平台，既是出处也方便自己验证。AI 生成图只用于没有具体引用对象的"氛围图"位置。封面图仍用 AI 生成。文章交付时必须附上截图清单（链接 + 对应位置）
