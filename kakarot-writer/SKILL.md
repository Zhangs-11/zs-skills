...
完整文章除封面外，默认评估哪些段落适合用叙事插画帮助理解，并按需生成正文配图。正文叙事配图与封面统一优先使用 `baoyu-cover-image`，沿用已选封面的叙事插画方法、画风和回退规则。每张图对应一个段落的具体含义，表现人物动作、场景或关系，放在对应段落附近；数量随内容需要决定，不逐节凑图。具体制作与验收按 `references/delivery.md` 的“正文叙事配图”执行。
