...
- **配图优先用真实截图**。凡是引用了推文、GitHub issue、产品页面、研究报告的段落，用 `[插图：用户截图 - XXX]` 占位并提供原始链接，让用户自己截图。截图下方紧跟一行引用格式的来源地址，地址前必须加类型说明前缀，如 `> GitHub地址：github.com/obra/superpowers/issues/190`、`> 推文地址：x.com/xxx/status/xxx`、`> 公告地址：x.ai/news/xxx`、`> 原文地址：xxx`，既是出处也方便读者自己验证。AI 生成图只用于没有具体引用对象的"氛围图"位置。封面图仍用 AI 生成。文章交付时必须附上截图清单（链接 + 对应位置）
